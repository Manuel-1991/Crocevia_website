...
create table if not exists prenotazioni (
  ...
  categoria           text, -- copiata da servizi.categoria via trigger, serve al vincolo sotto
  ...
  -- pensione/asilo: un solo cane alla volta in tutta la struttura
  -- (per aumentare la capienza in futuro: si toglie questo vincolo e si
  -- sostituisce con un controllo via trigger che conta le prenotazioni attive)
  exclude using gist (
    daterange(data_dal, data_al, '[]') with &&
  ) where (stato in ('in_attesa_pagamento','confermata') and categoria in ('pensione','asilo'))
);

create or replace function imposta_categoria_prenotazione()
returns trigger
language plpgsql as $$
begin
  select categoria into new.categoria from servizi where id = new.servizio_id;
  return new;
end;
$$;

drop trigger if exists trg_imposta_categoria on prenotazioni;
create trigger trg_imposta_categoria
  before insert or update of servizio_id on prenotazioni
  for each row execute function imposta_categoria_prenotazione();
...
create or replace view disponibilita_pensione_asilo as
  select categoria, data_dal, data_al
  from prenotazioni
  where stato in ('in_attesa_pagamento','confermata')
    and categoria in ('pensione','asilo');
